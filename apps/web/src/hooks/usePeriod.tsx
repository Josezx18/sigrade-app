...
export function PeriodProvider({ children }: { children: ReactNode }) {
  ...
  const fetchPeriods = useCallback(async () => {
    try {
      ...
      const periodList = Array.isArray(data) ? data : [];
      setPeriods(periodList);

      const periodIdFromUrl = searchParams.get('periodId');
      if (periodIdFromUrl) {
        const found = periodList.find((p: Period) => p.id === periodIdFromUrl);
        if (found) {
      ...
      const active = periodList.find((p: Period) => p.isActive);
      if (active) {
